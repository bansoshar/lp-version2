...
const SectionHeader = ({ en, jp, align = "center" }: { en: string; jp: string; align?: "left" | "center" | "right" }) => (
  <div className={`mb-20 flex flex-col ${align === "left" ? "items-start" : align === "right" ? "items-end" : "items-center"}`}>
    <motion.span 
...
const FAQItem = ({ q, a }: { q: string; a: string }) => {
  ...
  return (
    <div className="border-b border-slate-200">
      ...
      >
        <span className="font-serif text-lg font-medium pr-8 text-left leading-relaxed">{q}</span>
        <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''} flex-shrink-0 ml-4`} />
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div 
...
export default function Home() {
  ...
  const openForm = () => window.open("https://docs.google.com/forms/d/e/1FAIpQLSf4yPuDTIVX5emBJKNv4aNoP_sBO7yCTEJUju4cy3LCCngzaQ/viewform", "_blank");
  ...
  return (
    <div className="relative w-full">
      ...
      {/* --- Navigation --- */}
      <nav className={`fixed top-0 left-0 w-full p-6 md:p-10 flex justify-between items-center z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-sm shadow-sm py-4' : 'bg-transparent'}`}>
        
        {/* ロゴ部分: テキスト + 画像 */}
        <div className={`flex items-center gap-3 font-serif text-xl font-bold tracking-widest transition-colors duration-300 ${scrolled ? 'text-[#223a5e]' : 'text-white'}`}>
          <span>NAKAIMA BANSOSHAR</span>
          <img 
            src="/logo.png" 
            alt="Logo" 
            className={`h-8 w-auto ml-[-10px] transition-opacity duration-300 ${scrolled ? 'opacity-100' : 'opacity-0'}`}
          />
        </div>

        <button 
          onClick={() => setMenuOpen(!menuOpen)}
          className={`group flex items-center gap-3 text-sm tracking-widest uppercase hover:opacity-70 transition-opacity ${scrolled ? 'text-[#223a5e]' : 'text-white'}`}
        >
          <span className="hidden md:block">Menu</span>
          <div className={`w-8 h-[1px] transition-all duration-300 group-hover:w-12 ${scrolled ? 'bg-[#223a5e]' : 'bg-white'}`} />
        </button>
      </nav>
        ...
        {menuOpen && (
          ...
          >
            ...
            <ul className="space-y-8 text-center font-serif text-3xl md:text-5xl">
              {['Impact', 'Profile', 'Services', 'Process', 'Voices', 'FAQ'].map((item, i) => (
                <motion.li 
                  key={item}
                ...
                >
                  <a href={`#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)} className="hover:text-[var(--color-paper)] transition-colors">
                    {item}
                  </a>
                </motion.li>
              ))}
      ...
      <section id="impact" className="py-32 px-6 md:px-20 relative bg-white">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-16 items-start">
          <div className="md:w-1/3 relative md:sticky md:top-32">
            <span className="font-display text-6xl text-slate-100 absolute -top-10 -left-10 -z-10">01</span>
            <h2 className="font-serif text-3xl font-bold mb-6 text-[#223a5e]">あなたを待つ変化</h2>
            <div className="leading-loose text-gray-600 mb-8">
              <p>
                <span className="inline-block">ナカイマ伴走舎のコーチングは、</span><br className="hidden md:inline"/>
                <span className="inline-block">単なる悩み相談ではありません。</span>
              </p>
              <p className="mt-4">
                <span className="inline-block">自己理解を深め、</span>
                <span className="inline-block">視座を高めることで、</span><br className="hidden md:inline"/>
      ...
      {/* --- Profile --- */}
      <section id="profile" className="py-32 px-6 md:px-20 bg-[#223a5e] text-white overflow-hidden">
        <div className="max-w-7xl mx-auto relative">
          <div className="mb-20">
             <span className="font-display text-white/60 text-lg tracking-widest uppercase mb-4 block">02 ― The Coach</span>
             <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl leading-tight font-medium inline-block text-white">伴走者について</h2>
          </div>
        ...
        <div className="max-w-7xl mx-auto space-y-32">
          ...
          <div className="flex flex-col md:flex-row gap-12 md:gap-24 items-center group">
            ...
            <div className="w-full md:w-1/2">
              <h3 className="font-serif text-3xl font-bold mb-6 text-[#223a5e] group-hover:opacity-80 transition-opacity">
                ライフコーチング<br />
                <span className="text-lg font-normal font-sans text-gray-500 mt-2 block">(Co-Active®)</span>
              </h3>
              ...
              <ul className="space-y-2 mb-8 font-serif text-gray-700">
                <li className="flex items-center gap-3"><Check size={16} className="text-[#223a5e]"/> 真の願いと価値観の言語化</li>
                <li className="flex items-center gap-3"><Check size={16} className="text-[#223a5e]"/> メンタルブロックの解消</li>
                <li className="flex items-center gap-3"><Check size={16} className="text-[#223a5e]"/> 願いを行動へ、確かな一歩を</li>
              </ul>
              <MagneticButton dark={false} onClick={openForm}>View Details</MagneticButton>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row-reverse gap-12 md:gap-24 items-center group">
            ...
            <div className="w-full md:w-1/2">
              <h3 className="font-serif text-3xl font-bold mb-6 text-[#223a5e] group-hover:opacity-80 transition-opacity">
                ストレングスコーチング<br />
                <span className="text-lg font-normal font-sans text-gray-500 mt-2 block">(CliftonStrengths®)</span>
              </h3>
              <p className="text-gray-600 leading-loose mb-8">
                <span className="inline-phrase">「才能」は、磨かなければ</span>
                <span className="inline-phrase">ただの「癖」です。</span><br />
                <span className="inline-phrase">Gallup社の診断ツールを用いて、あなたの才能を客観的に分析。</span><br className="hidden md:inline" />
                <span className="inline-phrase">無意識の行動パターンを戦略的な「強み」へと昇華させ、</span><br className="hidden md:inline" />
                <span className="inline-phrase">パフォーマンスを最大化します。</span>
              </p>
              <ul className="space-y-2 mb-8 font-serif text-gray-700">
                <li className="flex items-center gap-3"><Check size={16} className="text-[#223a5e]"/> 資質プロファイルの徹底分析</li>
                <li className="flex items-center gap-3"><Check size={16} className="text-[#223a5e]"/> 卓越した成果を生む「勝ちパターン」の活用</li>
                <li className="flex items-center gap-3"><Check size={16} className="text-[#223a5e]"/> 強みを活かしたマネジメント</li>
              </ul>
              <MagneticButton dark={false} onClick={openForm}>View Details</MagneticButton>
            </div>
          </div>
      ...
      <section id="voices" className="py-32 px-6 md:px-20 bg-[var(--color-paper-dark)]">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {[
            { text: "最初は不安でしたが、同じ立場で肯定的に関わってもらえて安心できました。気づけばたくさん話していて、最後にはきれいに整理されていました。", user: "30代男性 / サービス業" },
            { text: "自分を知る良い機会でした。数回のセッションを経て、日々の選択に迷いが減り、自信を持って行動できるようになりました。", user: "30代女性 / フリーランス" },
            { text: "悩んだ時にセッションを思い出すことで気持ちを切り替えられるようになりました。普段の仕事や生活でも、自分を整える支えになっています。", user: "40代男性 / IT企画職" },
            { text: "自分の声の聴き方とモノの見方を深めることができました。どんな状態で居たいのかに気づけて、本当に感謝しています。", user: "30代男性 / 広告業界" }
          ].map((item, i) => (
            <motion.div 
              key={i}
              whileHover={{ y: -5 }}
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-bold text-gray-500 uppercase tracking-wider">{item.user}</span>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      {/* --- CTA / Footer --- */}
      <footer id="contact" className="bg-[#223a5e] text-white pt-32 pb-10 px-6 md:px-20 overflow-hidden relative">
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <motion.h2 
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            className="font-serif text-4xl md:text-6xl font-bold mb-12 leading-tight"
          >
            <span className="inline-phrase">すべての可能性は、</span><br />
            <span className="inline-phrase">この瞬間に。</span>
          </motion.h2>
          <p className="text-white/70 text-lg mb-16 max-w-2xl mx-auto leading-relaxed">
            <span className="inline-phrase">まずは30分の無料体験セッションで、</span><br className="hidden md:inline" />
            <span className="inline-phrase">あなたの中に眠る「願い」に触れてみてください。</span>
          </p>
          
          <div className="flex justify-center mb-32">
            <button 
              onClick={openForm}
              className="group relative inline-flex h-20 md:h-24 items-center justify-center overflow-hidden rounded-full bg-white px-10 md:px-16 font-medium text-[#223a5e] transition-all duration-300 hover:w-full hover:bg-[#223a5e] hover:text-white hover:border hover:border-white"
            >
              <span className="mr-4 text-lg md:text-xl font-bold tracking-widest">無料体験を申し込む</span>
